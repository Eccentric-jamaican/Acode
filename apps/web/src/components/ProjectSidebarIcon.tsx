...
export function ProjectSidebarIcon(props: { cwd: string; expanded: boolean }) {
  const faviconSrc = resolveProjectFaviconUrl(props.cwd);
  const [hasFavicon, setHasFavicon] = useState<boolean>(
    () => projectFaviconPresence.get(faviconSrc) === true,
  );
  const FolderGlyph = props.expanded ? HiOutlineFolderOpen : IoFolderOutline;
    ...
    <>
      ...
      {hasFavicon ? (
        <img
          ...
          alt=""
          aria-hidden="true"
          className="-bottom-1 -right-1 absolute size-3 rounded-[4px] object-contain shadow-sm"
          onError={() => {
            projectFaviconPresence.set(faviconSrc, false);
            setHasFavicon(false);
          }}
