    ...
    <CommandDialogPortal>
      ...
      <CommandDialogViewport>
        <CommandDialogPrimitive.Popup
          className={cn(
            "-translate-y-[calc(1.25rem*var(--nested-dialogs))] relative row-start-2 flex max-h-[min(420px,60vh)] min-h-0 w-full min-w-0 max-w-[560px] scale-[calc(1-0.1*var(--nested-dialogs))] flex-col rounded-xl border bg-popover not-dark:bg-clip-padding text-popover-foreground opacity-[calc(1-0.1*var(--nested-dialogs))] shadow-lg/5 outline-none transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-xl)-1px)] before:bg-muted/55 before:shadow-[0_1px_--theme(--color-black/4%)] data-nested:data-ending-style:translate-y-8 data-nested:data-starting-style:translate-y-8 data-nested-dialog-open:origin-top data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 **:data-[slot=scroll-area-viewport]:data-has-overflow-y:pe-1 dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            className,
          )}
...
function CommandPanel({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "-mx-px not-has-[+[data-slot=command-footer]]:-mb-px relative min-h-0 rounded-t-lg not-has-[+[data-slot=command-footer]]:rounded-b-xl border border-b-0 bg-popover/95 bg-clip-padding shadow-xs/5 [clip-path:inset(0_1px)] not-has-[+[data-slot=command-footer]]:[clip-path:inset(0_1px_1px_1px_round_0_0_calc(var(--radius-xl)-1px)_calc(var(--radius-xl)-1px))] before:pointer-events-none before:absolute before:inset-0 before:rounded-t-[calc(var(--radius-lg)-1px)] **:data-[slot=scroll-area-scrollbar]:mt-1",
        className,
      )}
      {...props}
    />
...
function CommandItem({ className, ...props }: React.ComponentProps<typeof AutocompleteItem>) {
  return (
    <AutocompleteItem className={cn("min-h-7 py-1", className)} data-slot="command-item" {...props} />
  );
}
...
function CommandSeparator({
  ...
  return (
    <AutocompleteSeparator
      className={cn("my-1.5 bg-border/60", className)}
...
function CommandShortcut({ className, ...props }: React.ComponentProps<"kbd">) {
  return (
    <kbd
      className={cn(
        "ms-auto font-sans text-[10px] text-muted-foreground/50 tracking-wide",
        className,
      )}
...
function CommandFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "flex items-center justify-between gap-2 rounded-b-[calc(var(--radius-xl)-1px)] border-t border-border/60 bg-muted/30 px-4 py-2 text-[11px] text-muted-foreground/60",
        className,
      )}
