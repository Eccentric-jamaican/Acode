...
function resolveClaudeModelConfig(input: {
  ...
  readonly modelOptions?: { readonly claudeAgent?: ClaudeModelOptions | undefined } | undefined;
}) {
  const model = trimOrNull(input.model) ?? undefined;
  const options = normalizeClaudeModelOptions(model, input.modelOptions?.claudeAgent);
  return {
    model,
    options,
  };
...
function buildPromptText(input: ProviderSendTurnInput): string {
  ...
  const rawEffort = modelConfig.options?.effort ?? null;
  const requestedEffort = trimOrNull(rawEffort);
  const claudeModel = modelConfig.model;
  ...
  return Effect.gen(function* () {
    ...
    const startSession: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const modelConfig = resolveClaudeModelConfig(input);
        const requestedEffort = trimOrNull(modelConfig.options?.effort ?? null);
        const caps = getModelCapabilities("claudeAgent", modelConfig.model);
        const effort =
          requestedEffort && hasEffortLevel(caps, requestedEffort) ? requestedEffort : null;
        const fastMode = modelConfig.options?.fastMode === true && caps.supportsFastMode;
        const thinking =
          typeof modelConfig.options?.thinking === "boolean" && caps.supportsThinkingToggle
      ...
      Effect.gen(function* () {
        ...
        if (modelConfig.model) {
          yield* Effect.tryPromise({
            try: () => context.query.setModel(modelConfig.model),
        ...
        yield* offerRuntimeEvent({
          ...
          threadId: context.session.threadId,
          turnId,
          payload: modelConfig.model ? { model: modelConfig.model } : {},
          providerRefs: {},
        });
