...
import Migration from "./031_ProjectionThreadsArchivedAt.ts";
...
async function withTempSqlite<A>(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "t3-migration-031-"));
...
describe("031_ProjectionThreadsArchivedAt", () => {
  it("adds archived_at to projection_threads", async () => {
